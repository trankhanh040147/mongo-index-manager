                ...
                <Card>
                    ...
                    <CardBody>
                        <p className="text-muted">Use <code>nested-list nested-sortable</code> class to list-group class
                            to set a nested list with sortable items.</p>
                            ...
                            <ReactDragListView {...dragProps}>
                                {this.state.data.map((item, index) => (
                                    <React.Fragment key={index}>
                                        {!(item.set1) ?
                                            <ListGroupItem className={item.class}>{item.title}</ListGroupItem> :
                                            <ListGroupItem className={item.class}>{item.title}
                                                <ListGroup className="nested-list nested-sortable">
                                                    {item.set1.map((item, key) => (
                                                        <React.Fragment key={key}>
                                                            {!(item.set2) ? <ListGroupItem
                                                                    className={item.class}>{item.title}</ListGroupItem> :
                                                                <ListGroupItem className={item.class}>{item.title}
                                                                    <ListGroup className="nested-list nested-sortable">
