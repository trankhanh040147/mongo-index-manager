        ...
        return (
            ...
            <Col lg={6}>
                <Card>
                    ...
                    <CardBody>
                        <p className="text-muted">Use <code>nested-list nested-sortable</code> class to list-group class
                            to set a nested list with sortable items where icons are given within list-group-item.</p>
                        <ListGroup className="col nested-list nested-sortable">
                            <ListGroupItem className="nested-1"> <i
                                className="mdi mdi-folder fs-16 align-middle text-warning me-2"></i> assets
                                <ListGroup className="nested-list nested-sortable">
                                    <ReactDragListView {...dragProps}>

                                        {this.state.data.map((item, key) => (
                                            <React.Fragment key={key}>
                                                {item.set1 ? <ListGroupItem className={item.class}> <i
                                                        className={"fs-16 align-middle text-" + item.iconClass + " me-2 " + item.icon}></i> {item.title}
                                                    </ListGroupItem>
                                                    : <ListGroupItem className={item.class}> <i
                                                        className={"fs-16 align-middle text-warning me-2 " + item.icon}></i> {item.title}
                                                        <ListGroup className="nested-list nested-sortable">
                                                            {item.set2.map((item, key) => (
                                                                <ListGroupItem className={item.class} key={key}><i
                                                                    className={"fs-16 align-middle text-" + item.iconClass + " me-2 " + item.icon}></i> {item.title}
                                                                </ListGroupItem>))}
                                                        </ListGroup>
                                                    </ListGroupItem>}
