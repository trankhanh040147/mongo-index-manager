...
const Collections = () => {
    document.title = "Collections | DRManager - React Admin & Dashboard Template";
    return (
        <React.Fragment>
            <div className="page-content">
                <Container fluid>
                                ...
                                <CardBody>
                                    <Row className="g-1 mb-3">
                                        {item.subItem.map((item, key) => (<Col className="col-6" key={key}>
                                            {item.isChildren.map((item, key) => (<img src={item.img} alt=""
                                                                                      className={"img-fluid rounded " + item.imgClass}
                                                                                      key={key}/>))}
                                        </Col>))}
                                    </Row>
                                    <Link to="#!" className="float-end"> View All <i
                                        className="ri-arrow-right-line align-bottom"></i></Link>
                                    <h5 className="mb-0 fs-16"><Link to="#!" className="text-body">{item.category} <span
                                        className="badge bg-success-subtle text-success">{item.quantity}</span></Link>
                                    </h5>
                                </CardBody>
                            </Card>
                        </Col>))}
                    ...
                    <Row className="g-0 text-center text-sm-start align-items-center mb-4">
                        <Col sm={6}>
                            <div>
                                <p className="mb-sm-0 text-muted">Showing <span
                                    className="fw-semibold">1</span> to <span className="fw-semibold">8</span> of <span
